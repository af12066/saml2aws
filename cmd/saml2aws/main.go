...
var CLI struct {
	Configure struct {
		AppID                 string `cmd:"" help:"OneLogin app id required for SAML assertion." env:"ONELOGIN_APP_ID"`
		ClientID              string `cmd:"" help:"OneLogin client id, used to generate API access token." env:"ONELOGIN_CLIENT_ID"`
		ClientSecret          string `cmd:"" help:"OneLogin client secret, used to generate API access token." env:"ONELOGIN_CLIENT_SECRET"`
		Subdomain             string `cmd:"" help:"OneLogin subdomain of your company account." env:"ONELOGIN_SUBDOMAIN"`
		MfaIpAddress          string `cmd:"" help:"IP address whitelisting defined in OneLogin MFA policies." env:"ONELOGIN_MFA_IP_ADDRESS"`
		Profile               string `cmd:"" help:"The AWS profile to save the temporary credentials." env:"SAML2AWS_PROFILE"`
		ResourceID            string `cmd:"" help:"F5APM SAML resource ID of your company account." env:"SAML2AWS_F5APM_RESOURCE_ID"`
		CredentialsFile       string `cmd:"" help:"The file that will cache the credentials retrieved from AWS. When not specified, will use the default AWS credentials file location." env:"SAML2AWS_CREDENTIALS_FILE"`
		CacheSaml             string `cmd:"" help:"Caches the SAML response" env:"SAML2AWS_CACHE_SAML"`
		CacheFile             string `cmd:"" help:"The location of the SAML cache file" env:"SAML2AWS_SAML_CACHE_FILE"`
		DisableSessions       string `cmd:"" help:"Do not use Okta sessions. Uses Okta sessions by default." env:"SAML2AWS_OKTA_DISABLE_SESSIONS"`
		DisableRememberDevice string `cmd:"" help:"Do not remember Okta MFA device. Remembers MFA device by default." env:"SAML2AWS_OKTA_DISABLE_REMEMBER_DEVICE"`
	} `cmd:""`
	Version kong.VersionFlag `help:"Show application version."`
}
